...
void change_attackrate(Bitu regbase, op_type* op_pt) {
	...
	if (attackrate) {
		fltype f = (fltype)(powf(FL2,(fltype)attackrate+(op_pt->toff>>2)-1)*attackconst[op_pt->toff&3]*recipsamp);
...
void change_decayrate(Bitu regbase, op_type* op_pt) {
	...
	if (decayrate) {
		...
		op_pt->decaymul = (fltype)(powf(FL2,f*powf(FL2,(fltype)(decayrate+(op_pt->toff>>2)))));
...
void change_releaserate(Bitu regbase, op_type* op_pt) {
	...
	if (releaserate) {
		fltype f = (fltype)(-7.4493*decrelconst[op_pt->toff&3]*recipsamp);
		op_pt->releasemul = (fltype)(powf(FL2,f*powf(FL2,(fltype)(releaserate+(op_pt->toff>>2)))));
...
void change_sustainlevel(Bitu regbase, op_type* op_pt) {
	...
	if (sustainlevel<15) {
		op_pt->sustain_level = (fltype)(powf(FL2,(fltype)sustainlevel * (-FL05)));
	} else {
...
void change_feedback(Bitu chanbase, op_type* op_pt) {
	...
	if (feedback) op_pt->mfbi = (Bit32s)(powf(FL2,(fltype)((feedback>>1)+8)));
	else op_pt->mfbi = 0;
}
...
void change_frequency(Bitu chanbase, Bitu regbase, op_type* op_pt) {
	...
	op_pt->vol = (fltype)(powf(FL2,(fltype)(vol_in * -0.125 - 14)));
...
void adlib_init(Bit32u samplerate) {
	...
	for (i=0; i<TREMTAB_SIZE; i++) {
		...
		trem_table[i] = (Bit32s)(powf(FL2,trem_val1)*FIXEDPT);
		trem_table[TREMTAB_SIZE+i] = (Bit32s)(powf(FL2,trem_val2)*FIXEDPT);
	}
	...
	if (!initfirstime) {
		...
		for (i=0;i<(WAVEPREC>>1);i++) {
			wavtable[(i<<1)  +WAVEPREC]	= (Bit16s)(16384*sinf((fltype)((i<<1)  )*PI*2/WAVEPREC));
			wavtable[(i<<1)+1+WAVEPREC]	= (Bit16s)(16384*sinf((fltype)((i<<1)+1)*PI*2/WAVEPREC));
